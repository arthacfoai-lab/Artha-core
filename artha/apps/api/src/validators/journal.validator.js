...
const journalLineSchema = Joi.object({
  ...
  ledgerId: Joi.string()
    ...
    .messages({
      'any.required':
        'ledgerId is required for each journal line',

      'string.guid':
        'ledgerId must be a valid UUID',
    }),
   ...
   *
   * We want lowercase values like:
   *   dr
   *   cr
   *
   * to FAIL validation instead of auto-transforming.
   */
  ...
  amount: Joi.number()
    ...
    .messages({
      ...
      'number.integer':
        'amount must be an integer (paise) — no decimals allowed',

      'number.min':
        'amount must be at least 1 paise',

      'number.max':
        'amount exceeds maximum allowed per line (₹1 crore)',

      'any.required':
        'amount is required for each journal line',
    }),
...
const createJournalSchema = Joi.object({
  ...
  lines: Joi.array()
    ...
    .messages({
      ...
      'array.min':
        'Journal entry requires at least 2 lines (double-entry)',
